...
> **核心理念**：AI 不是替代产品经理，而是将产品经理从重复性工作中解放出来，专注于决策和判断。
>
> **适用场景**：技术对接类项目（如支付网关、API 集成、第三方服务对接）
>
> **最后更新**：2026-05-29
...
**核心问题**：AI 的训练数据有截止日期，且不包含你公司的私有文档。
...
| 容易产生幻觉（hallucination） | 基于真实文档生成，可溯源 |
...
> ```
>
> **说明**：由于 MCP 配置格式问题，这个 prompt 尝试了 4 次才成功。关键教训：
...
> ```
>
> **产出**：`src/verify-card-payments.ts`（验证脚本）
...
> ```
>
> **AI 回应**：解释 entity 是 Worldpay 在入驻时分配的商户实体引用，建议先用 `default` 试
>
> **后续 Prompt**：
> ```
...
> ```
>
> **产出**：`src/probe-entities.ts`（自动探测脚本），发现 `gfhk001` 可用
...
>
> **说明**：
> 1. 使用 subagent 进行并行审查，不阻塞主流程
> 2. 指定模型（mimo-v2.5-pro）确保审查质量
> 3. 要求使用多个信息源交叉验证：
>    - Worldpay 官方文档
>    - Stripe API 文档
>    - 社区最佳实践
...
**模板**：
```
读取 [设计文档路径] 和 [PRD 路径]，转换为 GitHub Issues：

要求：
1. 每个 Issue 是一个垂直切片（tracer bullet）
2. 包含依赖关系图
3. 每个 Issue 有明确的验收标准
4. 标注 AFK/HITL 类型

输出：GitHub Issues（使用 gh issue create）
```
...
4. all AFK
5. ok
we have not decided the tech stack I think. use nextjs-fullstack + prisma
```

**说明**：AI 会先提出 9 个垂直切片方案，用户确认后再执行。这种「先对齐，再执行」的模式非常重要。

**输出物摘录**（GitHub Issues 创建）：
```bash
# Issue 1: Scaffold & Auth
gh issue create --title "1. Scaffold & Auth — Monorepo setup, API key auth" \
  --body '## What to build
Bootstrap the PayFac Payment Gateway monorepo...

### Tech stack
- **Monorepo:** Turborepo + pnpm workspaces
- **App:** apps/gateway — Next.js 16 App Router
- **Database:** Prisma v7 with PostgreSQL

## Acceptance criteria
- [ ] `pnpm install` succeeds across all workspaces
- [ ] `turbo dev` starts Next.js dev server
...
# Issue 3: CIT Card Payment
github issue create --title "3. CIT Card Payment — Core (no 3DS)" \
  --body '## What to build
Implement the core CIT payment flow through POST /api/v1/payment_intents...

## Acceptance criteria
- [ ] POST with type="card" returns 200 { status: "succeeded" }
- [ ] POST with type="card_token" returns 200 { status: "succeeded" }
- [ ] FraudSight assessment runs on every CIT payment
- [ ] PayFac paymentFacilitator block injected correctly
